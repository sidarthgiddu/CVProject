...
int MAX_SPEED_FRONT = 255; //max forward 
int MAX_SPEED_BACK = 0; //max backward

//linear angle for rotation rather than Radius
#define leftAngleMax -90  
#define rightAngleMax 90   
...
void thresh_callback(int, void*)
{
	...
	int beta = rangeConversion(leftAngleMax, rightAngleMax, l, lMin, lMax); // betaMin + ((betaMax - betaMin) * (l - lMin) / (lMax - lMin));
...
void iRobotCommandFormatVelocity(int velocitySpeed, int rotationAngle, char * stringDataOverNetwork) {
	...
	*(stringDataOverNetwork + 0) = (char)(velocitySpeed);
	*(stringDataOverNetwork + 1) = (char)(rotationAngle);
